		...
		private void MapProductSummaryItem(Product product, MapProductSummaryItemContext ctx)
		{
			...
			if (settings.MapPictures)
			{
				...
				item.Picture = new PictureModel
				{
					...
					Size = thumbSize,
					ImageUrl = pictureInfo.DirectUrl ?? _pictureService.GetUrl(pictureInfo, thumbSize, fallbackType),
					FullSizeImageUrl = pictureInfo.FullSizeDirectUrl ?? _pictureService.GetUrl(pictureInfo, 0, FallbackPictureType.NoFallback),
