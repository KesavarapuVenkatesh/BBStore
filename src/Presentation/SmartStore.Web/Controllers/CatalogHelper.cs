	...
	public partial class CatalogHelper
	{
		...
		private PictureModel CreatePictureModel(ProductDetailsPictureModel model, Picture picture, int pictureSize)
		{
			...
			var result = new PictureModel
			{
				PictureId = info?.Id ?? 0,
				Size = pictureSize,
				ThumbImageUrl = picture.DirectUrl ?? _pictureService.GetUrl(info, _mediaSettings.ProductThumbPictureSizeOnProductDetailsPage),
				ImageUrl = picture.DirectUrl ?? _pictureService.GetUrl(info, pictureSize, !_catalogSettings.HideProductDefaultPictures),
				FullSizeImageUrl = picture.FullSizeDirectUrl ?? _pictureService.GetUrl(info, 0, false),
